...
> **Não confundir** com o comando **`mb alias`** do CLI: *aliases* no manifest são atalhos de **nome de segmento** dentro da árvore de plugins; **[`mb alias`](../commands/alias.mdx)** são atalhos **pessoais** (comando + argumentos guardados em `aliases.yaml`, integração com o perfil do shell e uso com `mb run`).
