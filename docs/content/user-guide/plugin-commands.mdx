...
Na folha com **entrypoint** ou **flags**, o nome do subcomando no CLI vem do campo **`command`** do `manifest.yaml` (o último segmento do caminho interno continua a ser o nome da pasta). Plugins **só com `flags`** (sem entrypoint raiz) precisam de uma **flag declarada** para executar um script — ex.: `mb tools do --deploy`.
...
> **Não confundir** com o comando **`mb alias`** do CLI: *aliases* no manifest são atalhos de **nome de segmento** dentro da árvore de plugins; **[`mb alias`](../commands/alias.mdx)** são atalhos **pessoais** (comando + argumentos em **`aliases.yaml`** com integração no perfil do shell **e** `mb run`, ou em **`mbcli.yaml`** com `mb alias set --mbcli-yaml` — só **`mb run`**, sem funções em `shell/`).

**Cache:** os comandos vêm do SQLite após **`mb plugins sync`**. O `mb plugins add` dispara o sync; se editar arquivos diretamente em `PluginsDir`, execute novamente **`mb plugins sync`** para atualizar listagem, help e completion. Para detalhes sobre o fluxo de sync e digest de comandos, veja [Cache SQLite e sync](../technical-reference/plugins.mdx#cache-sqlite-e-sync).
