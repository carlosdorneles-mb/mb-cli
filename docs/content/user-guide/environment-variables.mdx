...
| 2 | **`env.defaults`** | Sempre — arquivo base | `~/.config/mb/env.defaults` |
...
- `aliases.test-all` → atalho em `mb run test-all` (sem vault extra); `aliases.staging.deploy` usa implicitamente `--env-vault staging` em `mb run deploy`, salvo se o mapa do alias definir `env_vault` — detalhes em [`mb alias`](../commands/alias.mdx#aliases-em-mbcliyaml-repositório)

### Secrets

#### Keyring do sistema (`--secret`)

O valor fica no keyring (macOS Keychain, Linux Secret Service), não em arquivo. Na listagem aparece como `***`. Use `mb envs list --show-secrets` para ver.
...
**Ver também:** [Guia `mbcli.yaml`](./mbcli-yaml.mdx) — resolução do caminho, chaves `envs` e `aliases`, e quais comandos alteram o ficheiro.

O arquivo `mbcli.yaml` na raiz do projeto pode definir variáveis sob a chave `envs`:
...
Para **criar, atualizar ou remover** variáveis nesse YAML a partir do terminal, use **`mb envs set … --mbcli-yaml`** e **`mb envs unset … --mbcli-yaml`** (veja [`mb envs set` / `unset`](../commands/envs.mdx)). Valores são sempre em texto no arquivo; não há `--secret` / `--secret-op` nesse modo.
...
| Atalho com vault só em `mb run` | [`mb alias set … --vault …`](../commands/alias.mdx) — o mesmo nome no shell não usa o merge do MB |
| Atalho compartilhado pela equipe (só `mb run`) | Chave `aliases` no [`mbcli.yaml`](../commands/alias.mdx#aliases-em-mbcliyaml-repositório) com `mb alias set --mbcli-yaml` |
| Variável compartilhada pela equipe | `envs` no `mbcli.yaml` (versionado); editar com [`mb envs set --mbcli-yaml`](../commands/envs.mdx) |
...
O primeiro token pode ser um **alias** criado com [`mb alias`](../commands/alias.mdx): o MB junta o **`aliases.yaml`** global (no [diretório de configuração do MB](../getting-started/installation.mdx#diretórios-por-sistema-operacional)) com a chave `aliases` do `mbcli.yaml` resolvido no repositório; se o **mesmo nome** existir nos dois, vale a entrada do **`mbcli.yaml`**. O vault salvo no alias entra no merge **a menos que** você informe `--env-vault` no `mb run` — nesse caso, o valor da linha de comando **prevalece**.
...
- [`mb alias`](../commands/alias.mdx) — Atalhos em `aliases.yaml` / perfil, em `mbcli.yaml` e `mb run` (com `env_vault` opcional)
