...
O primeiro token pode ser um **alias** criado com [`mb alias`](../commands/alias.mdx). O vault salvo no alias entra no merge **a menos que** você informe `--env-vault` no `mb run` — nesse caso, o valor da linha de comando **prevalece**.
...
- [`mb alias`](../commands/alias.mdx) — Atalhos no shell e `mb run` com `env_vault` opcional
