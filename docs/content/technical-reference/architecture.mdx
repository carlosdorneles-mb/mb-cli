...
Esta página descreve, em alto nível, como o MB CLI está organizado. Para detalhes de scanner, `groups.yaml` e cache, veja [Plugins](./plugins.mdx).
...
- Subcomandos **aninhados** (ex. `mb infra ci deploy`): por defeito **COMANDOS** (`commands`); se o manifest tiver **`group_id`** válido em **`plugin_help_groups`**, aparecem na secção com o título definido em `groups.yaml` (merge global no sync). Ver [Plugins — Grupos de help](./plugins.mdx#grupos-de-help-groupsyaml-group_id-e-cobra).
...
Para detalhes do scanner, validação, sync passo a passo e flags, veja [Plugins](./plugins.mdx).
