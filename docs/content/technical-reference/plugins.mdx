...
Esta página descreve como o MB CLI descobre, armazena e executa plugins — diretório de plugins, cache, sync e resolução de paths. Para **criar** um plugin e usar `mb plugins` no dia a dia, veja o [Guia: Criar um plugin](../plugin-authoring/create-a-plugin.mdx) e [Comandos de plugins](../user-guide/plugin-commands.mdx).
...
| Local (monorepo: sem `manifest.yaml` na raiz, manifests em `src/` ou `MB_PLUGIN_SUBDIR`) | Nome do diretório do repositório (ou `--package`) | `mb plugins add .` em `…/meu-repo` → um pacote `meu-repo` com `local_path` em `…/meu-repo/src`, como após instalar o mesmo repo por Git |
...
- **Ambiente:** sistema + defaults + `env_files` do plugin + `flags[].envs` (apenas das flags usadas) + `--env` + `MB_VERBOSE`/`MB_QUIET` + variáveis de contexto **`MB_CTX_*`** (invocação, path no manifest, irmãos Cobra, etc.).
  ...
  - Ver [Variáveis de ambiente](../user-guide/environment-variables.mdx), [Flags globais](../user-guide/global-flags.mdx) e [Contexto de invocação de plugins](plugin-invocation-context.mdx).
...
Os plugins correm com as permissões do utilizador; o CLI confina paths ao diretório do plugin no scan e no executor e suporta timeout opcional. Ver [Segurança](../user-guide/security.mdx).
