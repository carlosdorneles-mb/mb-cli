...
| `MB_CTX_CHILD_COMMAND_INFO` | JSON com um array de objetos `{"name":"…","description":"…"}` — mesmos filhos visíveis que `MB_CTX_CHILD_COMMANDS`; `description` é o texto curto Cobra (`Short`), sem espaços extremos; string vazia se não houver `Short`. |
...
- [Helpers de shell — context](../plugin-authoring/shell-helpers.mdx#context) — helper `context.sh`: `mb_context_dump`, `mb_context_dump_json`, `mb_peer_commands_lines`, `mb_child_commands_lines`, `mb_child_command_info_json`, `mb_hidden_child_commands_lines`, `mb_ctx_has_plugin_flag`, `mb_ctx_peer_contains`, `mb_ctx_child_contains`, `mb_ctx_hidden_child_contains`, `mb_ctx_peer_count`, `mb_ctx_child_count`, `mb_ctx_hidden_child_count`, `mb_ctx_cache_db`, `mb_ctx_parent_is`, `mb_ctx_command_path_is`, `mb_ctx_path_depth`
