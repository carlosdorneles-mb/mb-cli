...
Os arquivos são criados/atualizados pelo **`mb plugins sync`**. Se ainda não existirem, execute o sync antes.
...
| **`memory`** | Memória chave/valor entre execuções (arquivos em tmp) | Todas |
| **`os`** | Detecção de OS e distro Linux | Todas |
| **`mbcli-yaml`** | Lê/escreve `mbcli.yaml` do projeto (`envs`, etc.); o MB CLI também grava a chave **`aliases`** com `mb alias set --mbcli-yaml` / `mb alias unset --mbcli-yaml` | Todas |
...
Lê e altera o arquivo **`mbcli.yaml`** na raiz do projeto. Depende de **`yq`** (Mike Farah v4); use `ensure_yq` ou carregue `all.sh`.

**Resolução do caminho:** `MBCLI_YAML_PATH` → `${MBCLI_PROJECT_ROOT}/mbcli.yaml` → `${PWD}/mbcli.yaml`.

| Variável | Valores | Efeito |
|----------|---------|--------|
| `MBCLI_YAML_ON_MISSING` | `error` (default) / `ignore` | Com `ignore`, `get` sem arquivo retorna exit 0 e stdout vazio |
| `MBCLI_YAML_AUTOCREATE` | `0` (default) / `1` | Com `1`, `set` e `ensure` criam o arquivo (`{}`) se não existir |

| Função | Descrição |
|--------|-----------|
| `mbcli_yaml_path` | Imprime caminho resolvido do YAML |
| `mbcli_yaml_ensure` | Garante que o arquivo existe (com `AUTOCREATE=1`) |
...
O CLI também lê a chave **`envs`** no `mbcli.yaml` e mescla no ambiente do subprocesso, e a chave **`aliases`** para atalhos de repositório usados em **`mb run`** e em **`mb alias list`**. Para a lógica do ficheiro e comandos que o alteram, veja o [Guia `mbcli.yaml`](../user-guide/mbcli-yaml.mdx); para merge de ambiente e exemplos, [Variáveis de ambiente](../user-guide/environment-variables.mdx) e [`mb alias`](../commands/alias.mdx).
...
Blocos idempotentes com marcadores `# mb-cli-plugins:<tool>:begin` / `:end`. O corpo vive em **`${XDG_CONFIG_HOME:-$HOME/.config}/mb/shell-extras.sh`**; se `~/.bashrc` e/ou `~/.zshrc` **já existirem**, acrescenta-se um **stub** (uma linha de `source` desse arquivo) quando ainda não estiver presente. Não cria `~/.bashrc` / `~/.zshrc` automaticamente.
...
| `shell_rc_touch_extras_file` | Cria o diretório `mb` e o arquivo inicial se faltar |
| `shell_rc_ensure_stub` | Garante a linha `source` de `shell-extras.sh` nos RC existentes |
| `shell_rc_ensure_block MARKER_BEGIN MARKER_END BODY` | Garante stub e o arquivo de extras; acrescenta o bloco em `shell-extras.sh` (migra legado dos RCs se necessário) |
