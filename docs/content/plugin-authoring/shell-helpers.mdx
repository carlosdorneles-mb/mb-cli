...
O CLI também lê a chave **`envs`** no `mbcli.yaml` e mescla no ambiente do subprocesso, e a chave **`aliases`** para atalhos de repositório usados em **`mb run`** e em **`mb alias list`**. Veja [Variáveis de ambiente](../user-guide/environment-variables.mdx) e [`mb alias`](../commands/alias.mdx).
