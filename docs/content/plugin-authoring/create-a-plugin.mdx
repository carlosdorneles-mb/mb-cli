...
- **Referência técnica** (scanner, sync, grupos de help): [Plugins](../technical-reference/plugins.mdx)
- **Helpers de shell** (log, memória, k8s, etc.): [Helpers para plugins](./shell-helpers.mdx)
...
O `group_id` deve corresponder a um `id` definido num ficheiro **`groups.yaml`** do mesmo pacote. Comandos **top-level** (logo abaixo de `mb`) ignoram `group_id`. Veja [Grupos de help](../technical-reference/plugins.mdx#grupos-de-help-groupsyaml-group_id-e-cobra).
...
Para detalhes completos sobre subdiretórios e variável `MB_PLUGIN_SUBDIR`, veja [Subdiretório de plugins](../technical-reference/plugins.mdx#subdiretorio-de-plugins).

### Nome do pacote

O identificador do pacote segue as regras definidas em [Nome do pacote](../technical-reference/plugins.mdx#nome-do-pacote-identificador). Resumidamente:
...
O plugin recebe variáveis mescladas (sistema, defaults, vault, `--env`). Veja [Variáveis de ambiente](../user-guide/environment-variables.mdx) para a ordem de precedência.

Além disso, variáveis de contexto são injetadas (`MB_CTX_*`): path do comando, comando pai, irmãos, filhos, etc. Veja [Contexto de invocação](../technical-reference/plugin-invocation-context.mdx).
...
Lista completa: [Helpers para plugins](./shell-helpers.mdx).

### Códigos de saída

Para plugins com `install.sh` / `update.sh` que podem precisar de `sudo`, siga a [convenção de códigos de saída](../technical-reference/plugins.mdx#plugin-shell-exit-codes-convention):
...
- [Helpers de shell](./shell-helpers.mdx) — log, memória, OS, k8s, snap, homebrew…
- [Comandos de plugins](../user-guide/plugin-commands.mdx) — `mb plugins list`, `remove`, `update`
- [Variáveis de ambiente](../user-guide/environment-variables.mdx) — vaults, secrets, precedência
- [Plugins (referência técnica)](../technical-reference/plugins.mdx) — scanner, cache, sync em detalhe
