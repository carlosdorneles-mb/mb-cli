...
Gerencia variáveis de ambiente globais injetadas em **plugins**, no comando **`mb run`** e (quando o alias tiver vault definido com **`mb alias set --vault`**) na execução **`mb run <nome>`** com um atalho definido em **[`mb alias`](./alias.mdx)**.

Para entender ordem de precedência, vaults de projeto e quando usar cada abordagem, veja [Variáveis de ambiente](../user-guide/environment-variables.mdx). Para o ficheiro **`mbcli.yaml`** como um todo (`envs`, `aliases`, comandos que o alteram), veja o [Guia `mbcli.yaml`](../user-guide/mbcli-yaml.mdx).
...
**Variáveis do `mbcli.yaml`:** o `mb envs list` também mostra as variáveis definidas no `mbcli.yaml` da raiz do projeto. Elas aparecem com vault **`project`** (escalares na raiz de `envs`) e **`project/<nome>`** (sub-mapas), e armazenamento **`projeto`**. Para **editar** essas entradas pelo CLI, use **`mb envs set … --mbcli-yaml`** e **`mb envs unset … --mbcli-yaml`** (mesma ideia que [`mb alias set --mbcli-yaml`](./alias.mdx)). Não use `--vault project` nem `project/…` com `--mbcli-yaml` — para a raiz do YAML omita `--vault`; para submapa use só o nome (ex.: `--vault staging`). Detalhes em [Vaults de projeto](../user-guide/environment-variables.mdx#project-vaults).
...
# Variáveis versionadas no mbcli.yaml (sem --vault = raiz de envs; --vault = submapa)
mb envs set API_BASE=https://api.example --mbcli-yaml --yes
mb envs set LOG_LEVEL=debug --mbcli-yaml --vault staging --yes
```
...
| Flag | Descrição |
|---|---|
| `--vault <nome>` | Sem `--mbcli-yaml`: grava em `.env.<nome>` em vez do padrão. Não aceita `project` nem prefixo `project/` (reservados). Com **`--mbcli-yaml`**: grava em `envs.<nome>` no `mbcli.yaml` (submapa); não use `project` nem `project/` como nome de vault — use sem `--vault` para a raiz de `envs` |
| `--mbcli-yaml` | Grava na chave **`envs`** do `mbcli.yaml` do repositório (caminho: `MBCLI_YAML_PATH` / `MBCLI_PROJECT_ROOT` / cwd). **Não** combina com `--secret` nem `--secret-op` (só valores em texto no YAML). Regravar o YAML pode alterar ordem de chaves e não preserva comentários |
...
| `--yes` | Confirma sem prompt: `--secret-op` no vault padrão, ou alterações em `mbcli.yaml` com `--mbcli-yaml` (CI) |

> `--secret` e `--secret-op` são mutuamente exclusivos. **`--mbcli-yaml`** é mutuamente exclusivo com **`--secret`** e **`--secret-op`**. Sem flags de store, a variável `MB_ENVS_SECRET_STORE` (`plain` / `keyring` / `op`) no ambiente define o destino.
...
mb envs unset OLD_API --mbcli-yaml --yes
mb envs unset CACHE_TTL --mbcli-yaml --vault staging --yes
```

| Flag | Descrição |
|------|-----------|
| `--vault <nome>` | Com **`--mbcli-yaml`**: remove de `envs.<nome>` no YAML. Sem `--mbcli-yaml`: mesmo comportamento de antes (`.env.<nome>`) |
| `--mbcli-yaml` | Remove só chaves em **`envs`** do `mbcli.yaml` |
| `--yes` | Com **`--mbcli-yaml`**, confirma remoção sem prompt (CI) |

Com **`--mbcli-yaml`**, todas as chaves precisam existir no alvo; uma confirmação em lote (use **`--yes`** fora do TTY). Erros listam chaves inexistentes sem alterar o arquivo (comportamento diferente do modo em disco).

Sem `--mbcli-yaml`: se a chave não existir, exit code 0. Se não restar conteúdo num vault explícito, os arquivos `.env.<vault>`, `.secrets` e `.opsecrets` são apagados. `env.defaults` nunca é removido.
...
**Output:** tabela **VAULT** / **ARQUIVO** / **ENVS**. Inclui `default`, vaults em disco `.env.<nome>` no diretório de configuração (ex.: `~/.config/mb/.env.staging` no Linux), e linhas `project` / `project/<nome>` quando `mbcli.yaml` tem `envs`. O arquivo `.env.project` é ignorado (nome reservado).
...
`mb envs unset` (com o mesmo `--vault`, sem `--mbcli-yaml`) remove a chave dos arquivos, `.secrets`, `*.opsecrets`, keyring e item 1Password.
