...
| Flag | Descrição |
|---|---|
| `--package <id>` | Identificador do pacote. Se omitido, usa o nome do repositório ou do diretório. Para regras de nomenclatura, veja [Nome do pacote](../technical-reference/plugins.mdx#nome-do-pacote-identificador). |
| `--tag <tag>` | Instalar tag específica (remoto) |

**Subdiretório de plugins:** o MB detecta automaticamente se os plugins estão em `src/` (ou o valor de `MB_PLUGIN_SUBDIR`). Para detalhes, veja [Subdiretório de plugins](../technical-reference/plugins.mdx#subdiretorio-de-plugins).
...
Para detalhes sobre como criar plugins, veja [Criar um plugin](../plugin-authoring/create-a-plugin.mdx).
