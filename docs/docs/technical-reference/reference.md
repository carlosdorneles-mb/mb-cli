...
| `mb envs unset <KEY> [--group G]` | Remove de `env.defaults` ou de `.env.G` e, se for secret, do keyring (e 1Password quando usado). Se a chave não existir nesse grupo, mensagem informativa e saída **0** (nada alterado). |
