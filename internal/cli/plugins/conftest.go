...
func testAllPluginServicesWithDeps(
	t *testing.T,
) (*addplugin.Service, *usecaseplugins.SyncService, *usecaseplugins.RemoveService, *usecaseplugins.UpdateService, deps.Dependencies) {
	t.Helper()
	d := testPluginDeps(t)
...
func (testGitOps) GetVersion(
	string,
) (string, error) {
	return "1.0.0", nil
}

func (testGitOps) GetCurrentBranch(
	string,
) (string, error) {
	return "main", nil
}
...
func (testGitOps) ListLocalTags(
	string,
) ([]string, error) {
	return nil, nil
}

func (testGitOps) NewerTag(
	string,
	string,
) (string, bool) {
	return "", false
